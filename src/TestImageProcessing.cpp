...
namespace WYLEProcessing {
  static bool compareImages(const std::string& beforePath, const std::string& afterPath) {
      std::vector<uint8_t> beforeData = readFile(beforePath);
      std::vector<uint8_t> afterData = readFile(afterPath);
      ...
      for (size_t i = 0; i < beforeData.size(); ++i) {
          if (beforeData[i] != afterData[i]) {
              std::cerr << "Difference found at byte " << i << ": "
  ...
  static bool compareFileSizes(const std::string& beforePath, const std::string& afterPath) {
      ...
      std::cout << "File sizes are identical: " << beforeSize << " bytes." << std::endl;
      return true;
  }
